...
# Stage 1 alarm: explicit allowlist only. Do NOT use startswith("cases_") — that would
# include `cases_next4` (future sum), which is target leakage and drives proba → 1.0.
ALARM_STAGE1_FEATURE_COLUMNS: Tuple[str, ...] = (
    "cases_lag1",
    "cases_lag2",
    "cases_lag3",
    "cases_lag4",
...
def _compute_ww_detection_frequency_impl(ww: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
...
def _compute_ww_state_weekly_detection(ww: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Per-state weekly share of sites with measles detection (0–1), same QC rules as
    _compute_ww_detection_frequency_impl, so state scoring matches national WW semantics.
    """
    empty = pd.DataFrame(columns=["state", "year", "week", "ww_signal"])
    audit: Dict[str, Any] = {"signal_mode": None, "n_rows_after_filters": 0, "status": "empty"}
    if ww is None or ww.empty:
        return empty, audit
    ...
    if not year_col or not week_col or not site_col or not state_col:
    ...
    df["site_id"] = df[site_col].astype(str).str.strip()
    df["signal"] = _safe_numeric(df[signal_col])
    df["state"] = df[state_col].map(normalize_wastewater_jurisdiction_to_abbr)
    ...
    df = df.dropna(subset=["year", "week", "site_id", "state"])
    if df.empty:
        audit["status"] = "no_rows_after_qc"
        return empty, audit

    audit["n_rows_after_filters"] = len(df)
    df["is_detected"] = (df["signal"].fillna(0) > 0).astype(int)

    site_week = df.groupby(["site_id", "state", "year", "week"], as_index=False)["is_detected"].max()
    weekly = site_week.groupby(["state", "year", "week"], as_index=False).agg(
        total_sites=("site_id", "nunique"),
        positive_sites=("is_detected", "sum"),
    )
    weekly["ww_signal"] = weekly["positive_sites"] / weekly["total_sites"].clip(lower=1)
    weekly = weekly.sort_values(["state", "year", "week"]).reset_index(drop=True)
    audit["status"] = "ok"
    return weekly[["state", "year", "week", "ww_signal"]], audit


def get_national_ww_signal(ww: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    empty = pd.DataFrame(columns=["year", "week", "ww_signal"])
    detection_weekly, det_audit = _compute_ww_detection_frequency_impl(ww)
...
def get_state_ww_signal(ww: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    det_state, det_audit = _compute_ww_state_weekly_detection(ww)
    if not det_state.empty:
        audit.update(
            {
                **det_audit,
                "mode": "state_detection_frequency",
                "state_week_row_groups": int(len(det_state)),
                "signal_note": "Per-state site detection share (0–1); matches national WW pipeline semantics.",
            }
        )
        return det_state, audit
...
def build_modeling_frame(
    ...
    for _c in [c for c in df.columns if c.startswith("cases_lag") or c.startswith("cases_roll")]:
        df[_c] = np.log1p(_safe_numeric(df[_c]).clip(lower=0))

    if not ww_df.empty:
        tmp_ww = ww_df.copy()
        tmp_ww["year"] = _safe_numeric(tmp_ww["year"])
        tmp_ww["week"] = _clip_week(tmp_ww["week"])
        tmp_ww = tmp_ww.groupby(["year", "week"], as_index=False)["ww_signal"].mean()
        df = df.merge(tmp_ww, on=["year", "week"], how="left", validate="one_to_one")
    else:
        df["ww_signal"] = np.nan
...
def fit_alarm_model(modeling_df: pd.DataFrame) -> AlarmModelResult:
    ...
    feature_cols = [c for c in ALARM_STAGE1_FEATURE_COLUMNS if c in modeling_df.columns]
    if len(feature_cols) < len(ALARM_STAGE1_FEATURE_COLUMNS):
        return AlarmModelResult(None, None, [], 0.5, 0.25, 0, 0, "missing_feature_columns")
...
def _squash_extreme_alarm_proba(raw_proba: float, max_logit: float = 2.75) -> float:
    """
    Map saturated sklearn probabilities into a bounded range (~0.94 max at max_logit=2.75)
    so the dashboard does not show misleading 100% when the linear model extrapolates.
    """
    p = float(np.clip(raw_proba, 1e-9, 1.0 - 1e-9))
    logit = float(np.log(p / (1.0 - p)))
    logit = float(np.clip(logit, -max_logit, max_logit))
    return float(1.0 / (1.0 + np.exp(-logit)))


def _pipeline_predict_alarm_prob(model_result: AlarmModelResult, modeling_df: pd.DataFrame) -> float:
    ...
    try:
        raw = float(model_result.model.predict_proba(x_latest)[0, 1])
        ...
        try:
            ...
            xt = im.transform(x_latest)
            xs = sc.transform(xt)
            logit = float(lr.decision_function(xs)[0])
        except Exception:
            pass
        squashed = raw >= 0.97 or raw <= 0.03
        proba = _squash_extreme_alarm_proba(raw) if squashed else raw
        if squashed or (logit is not None and abs(logit) > 6.0):
            logger.warning(
                "Alarm raw_proba=%.4f displayed=%.4f logit=%s (squashed=%s). "
                "Extreme values often mean extrapolation beyond training case levels or class imbalance.",
                raw,
                proba,
                f"{logit:.2f}" if logit is not None else "n/a",
                squashed,
            )
        return proba
    except Exception:
...
def build_state_risk_index(
    ...
    peak_cases = float(coverage_df["cases_recent"].max()) if not coverage_df.empty else 0.0
    cov_raw = ((95.0 - coverage_df["coverage"]).clip(lower=0) * 2.0).clip(upper=50)
    if peak_cases < 5.0:
        coverage_df["coverage_points"] = (cov_raw * 0.35).clip(upper=18.0)
    else:
        coverage_df["coverage_points"] = cov_raw

    coverage_df["case_points"] = _pct_rank_points(coverage_df["cases_recent"], 30).fillna(0.0)
    ww_pos = coverage_df["ww_recent"].where(coverage_df["ww_recent"] > 0)
    coverage_df["wastewater_points"] = _pct_rank_points(ww_pos, 20).fillna(0.0)
    ...
    def _risk_tier_row(row: pd.Series) -> str:
        score = float(row["total_risk"])
        cr = float(row["cases_recent"])
        wwv = row["ww_recent"]
        ww_ok = pd.notna(wwv) and float(wwv) >= 0.05
        if score < 40:
            return "low"
        if score < 70:
            return "medium"
        if cr < 10 and not ww_ok:
            return "medium"
        return "high"

    coverage_df["risk_tier"] = coverage_df.apply(_risk_tier_row, axis=1)
...
def _pipeline_baseline_from_cases_df(
    cases_df: pd.DataFrame,
    projection_audit: Dict[str, Any],
) -> dict[str, Any]:
    """
    Baseline tier/score from Stage 2 projection audit + national weekly case line.
    """
    out: dict[str, Any] = {
        "tier": "low",
        "score": 0.0,
        "projection_baseline_weekly": None,
        "reference_median_weekly": None,
        "ratio": None,
        "recent_std": None,
        "formula": "",
        "method": "pipeline_stage2_vs_reference_median",
    }
    if cases_df is None or cases_df.empty or len(cases_df) < 8:
        out["formula"] = "Insufficient national weekly NNDSS history for Stage 2 baseline (need at least 8 weeks)."
        return out
    if projection_audit.get("status") != "ok":
        out["formula"] = "Stage 2 recency-weighted baseline projection unavailable for this window."
        return out
    baseline_mean = float(projection_audit["baseline_mean"])
    rs = projection_audit.get("recent_std")
    out["recent_std"] = round(float(rs), 2) if rs is not None else None
    c = _safe_numeric(cases_df["cases"])
    if len(c) > 12:
        ref = float(c.iloc[:-12].median())
    else:
        ref = float(c.median())
    if ref <= 0:
        ref = float(max(EPS, float(c.mean()) if c.mean() > 0 else EPS))
    ratio = baseline_mean / ref
    out["projection_baseline_weekly"] = round(baseline_mean, 2)
    out["reference_median_weekly"] = round(ref, 2)
    out["ratio"] = round(ratio, 2)
    if ratio > 1.5:
        out["tier"] = "high"
        out["score"] = min(100.0, 50.0 + (ratio - 1.5) * 50.0)
        out["formula"] = (
            "Ratio = (Stage 2 projection baseline mean weekly cases) ÷ (median weekly cases from earlier NNDSS, "
            "excluding the latest 12 weeks when enough history exists). Ratio > 1.5 → **high** tier; "
            "score = 50 + (ratio − 1.5)×50."
        )
    elif ratio > 1.0:
        out["tier"] = "medium"
        out["score"] = min(100.0, 30.0 + (ratio - 1.0) * 40.0)
        out["formula"] = (
            "Same ratio as above. Between 1.0 and 1.5 → **medium** tier; score = 30 + (ratio − 1)×40."
        )
    else:
        out["tier"] = "low"
        out["score"] = max(0.0, 20.0 * ratio)
        out["formula"] = "Ratio ≤ 1.0 → **low** tier; score = 20×ratio."
    out["score"] = round(float(out["score"]), 1)
    return out
...
def build_dashboard_payload(
    ...
    alarm_probability = _pipeline_predict_alarm_prob(model_result, modeling_df)
    projection_df, projection_audit = build_baseline_projection(cases_df)
    baseline_block = _pipeline_baseline_from_cases_df(cases_df, projection_audit)
    ...
    national_risk = {
        ...
        "baseline_tier": baseline_block["tier"],
        "baseline_score": baseline_block["score"],
        "baseline_projection_weekly": baseline_block.get("projection_baseline_weekly"),
        "baseline_reference_median_weekly": baseline_block.get("reference_median_weekly"),
        "baseline_ratio": baseline_block.get("ratio"),
    }
    ...
    diagnostics = {
        ...
        "notes": [
            ...
            "Baseline gauge score compares Stage 2 mean weekly baseline to median weekly NNDSS (earlier weeks, excluding latest 12 when possible).",
...
# -------------------------------------------------------------------
# Streamlit dashboard_v3 API (model_runner / app.py)
# -------------------------------------------------------------------
...
def get_outbreak_threshold_from_data(
    ...
    national, _ = get_national_weekly_cases(nndss)
...
def _outbreak_threshold_int(
    nndss: pd.DataFrame,
    outbreak_threshold: float,
    outbreak_percentile: Optional[float],
) -> int:
    if outbreak_percentile is not None:
        t = get_outbreak_threshold_from_data(nndss, outbreak_percentile)
        if t is not None:
            return max(1, int(round(t)))
    if outbreak_threshold and outbreak_threshold > 0:
        return max(1, int(round(outbreak_threshold)))
    return DEFAULT_ALERT_THRESHOLD


def _coef_df_from_alarm_result(model_result: AlarmModelResult) -> Optional[pd.DataFrame]:
    if model_result is None or model_result.model is None or not model_result.features:
        return None
    try:
        pipe = model_result.model
        ...
        if lr is None or not hasattr(lr, "coef_"):
            return None
        coef = lr.coef_.ravel()
        return pd.DataFrame({"feature": model_result.features, "coefficient": coef})
    except Exception:
        return None
...
def fit_stage1(
    ...
    wastewater: pd.DataFrame,
    kindergarten: pd.DataFrame,
    outbreak_threshold: float = 0,
    outbreak_percentile: Optional[float] = DEFAULT_OUTBREAK_PERCENTILE,
) -> Tuple[Optional[AlarmModelResult], Optional[pd.DataFrame], float, Optional[dict]]:
    thresh = _outbreak_threshold_int(nndss, outbreak_threshold, outbreak_percentile)
    modeling_df, _ = build_modeling_frame(nndss, wastewater, kindergarten, outbreak_threshold=thresh)
    result = fit_alarm_model(modeling_df)
    coef_df = _coef_df_from_alarm_result(result)
    metrics = {"auc": result.auc, "n_train": result.n_train, "n_test": result.n_test}
    if result.model is None:
        return None, coef_df, 0.5, None
    return result, coef_df, float(result.auc), metrics


def predict_alarm_probability(
    model: Optional[AlarmModelResult],
    nndss: pd.DataFrame,
    wastewater: pd.DataFrame,
    kindergarten: pd.DataFrame,
    outbreak_threshold: float = 0,
    ...
    thresh = _outbreak_threshold_int(nndss, outbreak_threshold, outbreak_percentile)
    modeling_df, _ = build_modeling_frame(nndss, wastewater, kindergarten, outbreak_threshold=thresh)
    return _pipeline_predict_alarm_prob(model, modeling_df)


def get_forecast(nndss: pd.DataFrame, weeks: int = FORECAST_WEEKS) -> Tuple[pd.DataFrame, bool]:
    cases_df, _ = get_national_weekly_cases(nndss)
    proj, audit = build_baseline_projection(cases_df, weeks=weeks)
    ok = audit.get("status") == "ok"
    if proj.empty or not ok:
        return pd.DataFrame(columns=["week_ahead", "forecast"]), False
    return proj[["week_ahead", "forecast"]].copy(), True


def _pipeline_baseline_from_nndss(nndss: Optional[pd.DataFrame]) -> dict[str, Any]:
    """
    Baseline tier/score for the dashboard gauge: Stage 2 recency-weighted weekly baseline
    vs median weekly cases from earlier NNDSS (excluding the latest 12 weeks when possible).
    """
    out: dict[str, Any] = {
        "tier": "low",
        "score": 0.0,
    ...
    if nndss is None or nndss.empty:
        out["formula"] = "No NNDSS data; baseline not computed."
        return out
    cases_df, _ = get_national_weekly_cases(nndss)
    if cases_df is None or cases_df.empty or len(cases_df) < 8:
        out["formula"] = "Insufficient national weekly NNDSS history for Stage 2 baseline (need at least 8 weeks)."
        return out
    _, audit = build_baseline_projection(cases_df)
    return _pipeline_baseline_from_cases_df(cases_df, audit)


def get_baseline_risk(nndss: pd.DataFrame) -> Tuple[str, float]:
    comp = _pipeline_baseline_from_nndss(nndss)
    return str(comp["tier"]), float(comp["score"])


def get_baseline_risk_components(nndss: pd.DataFrame) -> dict:
    return _pipeline_baseline_from_nndss(nndss)
...
def get_state_risk_df(
    ...
    ww = wastewater if wastewater is not None else pd.DataFrame()
    df, _ = build_state_risk_index(kindergarten, nndss, ww)
    if df.empty:
        return pd.DataFrame(
            columns=[
    ...
    out = df.copy()
    out["risk_score"] = out["total_risk"]
    out["wastewater_coverage"] = out["wastewater_available"]
    return out


def compute_ww_detection_frequency(
    ww_df: Optional[pd.DataFrame],
    year_min: Optional[int] = None,
    year_max: Optional[int] = None,
) -> Tuple[pd.DataFrame, dict]:
    raw = ww_df if ww_df is not None else pd.DataFrame()
    weekly, audit = _compute_ww_detection_frequency_impl(raw)
    n_raw = int(audit.get("n_rows_raw", 0))
    n_after = int(audit.get("n_rows_after_filters", 0))
    validation_info: dict = {
        "n_rows_raw": n_raw,
        "n_after_qc": n_after,
        "n_unique_sites": int(audit.get("n_unique_sites", 0)),
        "all_zero_freq": bool(audit.get("all_zero", True)),
        "pct_passing_qc": round(100.0 * n_after / n_raw, 1) if n_raw else 0.0,
        "weeks_min": audit.get("weeks_min"),
        "weeks_max": audit.get("weeks_max"),
        "pcr_target_used": None,
        "missing_columns": None,
        "columns_present": list(raw.columns) if not raw.empty else None,
        "detection_rule_used": audit.get("signal_mode"),
    }
    if not weekly.empty and (year_min is not None or year_max is not None):
        w = weekly.copy()
        w["year"] = pd.to_numeric(w["year"], errors="coerce")
        w = w.dropna(subset=["year"])
        if year_min is not None:
            w = w[w["year"] >= int(year_min)]
        if year_max is not None:
            w = w[w["year"] <= int(year_max)]
        weekly = w
...
def validate_ww_nndss_audit(
    ...
    logger.info(
        "audit: ww pct_passing_qc=%.1f unique_sites=%s all_zero_freq=%s nndss_cases_sum=%s",
        audit["ww_pct_passing_qc"],
        audit["ww_unique_sites"],
        audit["ww_all_zero_freq"],
        audit["nndss_cases_sum"],
    )
    return audit
