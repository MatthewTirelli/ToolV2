...
""",
    ...
    csv_mtime = _file_mtime(QC_CSV)
    summary_mtime = _file_mtime(QC_SUMMARY)
    df = _load_qc_results_csv(str(QC_CSV.resolve()), csv_mtime)
    summary_text = _load_qc_summary_md(str(QC_SUMMARY.resolve()), summary_mtime)

    if df is None:
        st.warning("No QC results are available yet.")
        st.caption("Run the offline QC protocol to generate `qc/outputs/qc_results.csv`.")
        _downloads_only()
