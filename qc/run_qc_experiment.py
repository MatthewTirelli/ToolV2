...
def main() -> None:
    ...
    parser.add_argument(
        "--omit-wide-columns",
        action="store_true",
        help="Drop report_text and grader_payload before writing CSV (keeps repo/git size reasonable).",
    )
    ...
    out_df = df
    if args.omit_wide_columns:
        drop_cols = [c for c in ("report_text", "grader_payload") if c in out_df.columns]
        if drop_cols:
    ...
    if args.omit_wide_columns:
        print("Note: qc_results.csv was written without report_text/grader_payload (--omit-wide-columns).", flush=True)
