...
from agents.state_risk_agent import _assistant_message_dict, _load_env
from agents.state_risk_agent import run_state_risk_forecaster
from agents.tools.report_context_tools import REPORT_CONTEXT_TOOL_NAME, REPORT_CONTEXT_TOOLS, execute_report_context_tool
from agents.tools.state_risk_snapshot import DISCLAIMER as SNAPSHOT_DISCLAIMER
...
# Agent 3 (final report): brief system line for tool contract only. Full briefing instructions live in
# qc/prompts/grounded_prompt.txt (same file as QC grounded mode; baseline prompt remains for QC only).
AGENT3_SYSTEM_BRIEF = (
    "You are a strict grounded report writer. On your first assistant turn, call get_precomputed_report_inputs "
    "exactly once; then write the final markdown using only the JSON returned by that tool."
)
...
def load_grounded_prompt() -> str:
    """Production default: same grounded template as QC (qc/prompts/grounded_prompt.txt)."""
    path = _repo_root() / "qc" / "prompts" / "grounded_prompt.txt"
    return path.read_text(encoding="utf-8")


def grounded_prompt_file_info() -> Dict[str, Any]:
    """Resolved path to the grounded prompt file (delegates to qc.live_validation_diagnostics)."""
    from qc.live_validation_diagnostics import grounded_prompt_file_info as _info

    return _info()


def _agent3_user_message_from_grounded_file() -> str:
    """Inject tool-based JSON delivery where QC passes inline JSON."""
    tpl = load_grounded_prompt()
    tool_block = (
        "Call the tool **get_precomputed_report_inputs** exactly once on your first turn. "
        "Use only the JSON in the tool response as the structured input for every rule in this prompt."
    )
    return tpl.replace("{structured_input_json}", tool_block)
...
def run_measles_report_agent(
    ...
    *,
    ...
    if not api_key:
        raise ValueError("OPENAI_API_KEY is not set; add it to repo or app/.env")
    ...
    messages: List[Dict[str, Any]] = [
        {"role": "system", "content": AGENT3_SYSTEM_BRIEF},
        {"role": "user", "content": _agent3_user_message_from_grounded_file()},
    ]
