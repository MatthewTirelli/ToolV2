...
from agents.tools.state_risk_tools import STATE_RISK_TOOL_NAME, STATE_RISK_TOOLS, execute_state_risk_tool

logger = get_logger("state_risk_agent")
...
def _load_env() -> None:
    ...
    load_dotenv(APP_ROOT / ".env", override=True, encoding="utf-8-sig")

    # Bulletproof: read app/.env directly so OPENAI_API_KEY is set even if load_dotenv skipped.
    dash = APP_ROOT / ".env"
    if dash.is_file():
        try:
